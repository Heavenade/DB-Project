...
<html>
  ...
  <body>
    ...
    if ($member[user_id])
    {
      echo $member[name]."(".$member[user_id].")님의 탐험 중 물가";
    }
    else {?>
      <script>
    ...
    $tmpquery2 = "select creature_name from creature where creature_id = '$tmp_creature_id'";
    $tmpresult2 = mysqli_query($connect,$tmpquery2);
    $tmpdata2 = mysqli_fetch_assoc($tmpresult2);
    $tmp_creature_name = $tmpdata2["creature_name"];

    $tmpquery2 = "select creature_type from creature where creature_id = '$tmp_creature_id'";
    $tmpresult2 = mysqli_query($connect,$tmpquery2);
    $tmpdata2 = mysqli_fetch_assoc($tmpresult2);
